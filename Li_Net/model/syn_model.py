...
class Multi_modal_generator(nn.Module):
    ...
    def __init__(self,input_nc, output_nc, ngf, num_classes):
        ...
        self.out1       = nn.Sequential(
            nn.Conv2d(int(self.out_dim),3, kernel_size=3, stride=1, padding=1),
            nn.Tanh()
        ) #  self.final_out_dim
    ...
    def forward(self,inputs):
        ...
        ## 添加的分类部分
        down_fu_4 = self.down_fu_4(down_fu_3m)
        output = self.avg_pool(down_fu_4)
        ...
        return output, output1, output2, down_fu_3m
